...
# shared, over HTTP
XAEL_AUTH_TOKEN=changeme xaelwiki --transport streamable-http --host 0.0.0.0 --port 8000
```
...
`deploy/xaelwiki.service` is a systemd unit: point it at the repo, set the
token, and `systemctl enable --now xaelwiki`. Terminate TLS with a reverse
...
- **Read-only mode.** `XAEL_READ_ONLY=1` removes every write tool from the
  surface.
- **Bearer auth** on the HTTP transport (skipped when no token is set).
