...
generates a bearer token and saves it to `~/.config/xaelwiki/env` (mode 600).
...
`deploy/xaelwiki.service` is a systemd unit: point it at the repo and set the
token in `~/.config/xaelwiki/env` (mode 600, generated by the installer),
then `systemctl enable --now xaelwiki`. Terminate TLS with a reverse proxy of
your choice.
